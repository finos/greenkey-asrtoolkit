...
def ordinal_to_string(input_string):
    ...
    if has_ordinal(input_string):
        ret_str = (
            (num2words.num2words(int(input_string[:-2]), ordinal=True))
            .replace(",", "")
            .replace("-", " ")
        )
...
def format_dollars_and_cents(input_string):
    ...
    # split into pieces
    dollars, cents = (
        input_string.split(".") if "." in input_string else [input_string, None]
    )
...
def digits_to_string(input_string):
    ...
    if input_string:
        ret_str = (
            num2words.num2words(int(input_string.split(".")[0]))
...
def pluralize(substring):
    ...
    def is_multiple_of_ten(notten):
        ...
        return (
            len(notten) == 2 and notten != "00" and notten != "10" and notten[-1] == "0"
        )
...
def plural_numbers_to_string(input_string):
    ...
    ret_str = (
        " ".join((digits_to_string(input_string[:2]), pluralize(input_string[2:])))
        if (len(input_string) == 4 and input_string[-2:] != "00")
        else pluralize(input_string)
    )
