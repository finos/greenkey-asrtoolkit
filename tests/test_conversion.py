...
def test_json_to_stm_conversion():
  ...
  new_sha = hashlib.sha1(open("tests/json_to_stm_test_2.stm", 'r', encoding='utf8').read().encode()).hexdigest()
  assert reference_sha == new_sha
...
def test_json_to_txt_conversion():
  ...
  new_sha = hashlib.sha1(open("tests/json_to_txt_test.txt", 'r', encoding='utf8').read().encode()).hexdigest()
  assert reference_sha == new_sha
