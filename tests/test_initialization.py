...
def test_json_initialization():
  ...
  text_object.write("file_conversion_test.stm")
...
def test_txt_initialization():
  ...
  reference_sha = hashlib.sha1(
    open("../samples/BillGatesTEDTalk_transcribed.txt", 'r', encoding='utf8').read().encode()
  ).hexdigest()
  text_object.write("file_conversion_test.txt")
